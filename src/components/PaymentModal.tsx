...
import { X, Copy, Check, Smartphone, Heart } from 'lucide-react';
        ...
        >
            ...
            >
                ...
                <div className="text-center mb-8">
                    ...
                    <h2 id="payment-modal-title" className="text-3xl font-black mb-2">Support PDFBolt</h2>
                ...
                <div className="flex flex-col items-center space-y-8">
                    ...
                    <div className={`p-4 rounded-[2rem] border-4 ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-indigo-50'}`}>
                        {qrUrl ? (
                            <img src={qrUrl} alt="Payment QR Code for UPI" className="w-56 h-56 rounded-2xl" />
                        ) : (
                            <div className="w-56 h-56 flex items-center justify-center text-xs text-slate-400 font-semibold">Generating QR...</div>
                        )}
                    </div>
                    ...
                    <a
                        href={UPI_URL}
                        className="w-full py-4 bg-[#5f259f] hover:bg-[#501e87] text-white rounded-xl font-black text-lg shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    <div className={`w-full p-4 rounded-xl flex items-center justify-between border ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                        ...
                        <button
                            type="button"
                            onClick={handleCopy}
                            aria-label={copied ? "Copied UPI ID" : "Copy UPI ID"}
